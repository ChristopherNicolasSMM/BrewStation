...
def _apply_profile_fields(user: User, data: Mapping[str, Optional[str]]) -> None:
    user.nome_completo = data.get("nome_completo") or data.get("fullName") or user.nome_completo
...
def update_profile_from_form(user: User, form_data: MutableMapping[str, str], files) -> None:
...
def update_notification_preferences_from_form(user: User, form_data: Mapping[str, str]) -> None:
...
def update_notification_preferences_from_json(user: User, payload: Mapping[str, object]) -> None:
    user.notificacao_alteracoes = bool(payload.get("notificacao_alteracoes", False))
    user.notificacao_novos_produtos = bool(payload.get("notificacao_novos_produtos", False))
    user.notificacao_ofertas = bool(payload.get("notificacao_ofertas", False))
    db.session.commit()
    logger.info("Preferências de notificação do usuário %s atualizadas (JSON).", user.username)


def change_password(user: User, senha_atual: str, nova_senha: str, confirmar_senha: str) -> None:
