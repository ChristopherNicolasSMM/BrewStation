...
def create_notification(user_id: int, message: str, title: Optional[str] = None, **kwargs) -> Notification:
...
def list_notifications(user_id: int, status: Optional[str] = None) -> List[Notification]:
...
def list_trash(user_id: int) -> List[NotificationTrash]:
    return NotificationTrash.query.filter_by(user_id=user_id).order_by(NotificationTrash.trashed_at.desc()).all()


def restore_from_trash(user_id: int, trash_id: int) -> Notification:
    trash_entry = NotificationTrash.query.filter_by(id=trash_id, user_id=user_id).first()
    if not trash_entry:
