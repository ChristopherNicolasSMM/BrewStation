...
def get_mqtt_service():
    ...
    Tenta primeiro pelo registry global, com fallback no atributo do plugin.
    """
    ...
    # Caminho 2: atributo direto do plugin
    try:
        plugin_manager = current_app.plugin_manager
        plugin = plugin_manager.get_plugin('plugin_device_manager')
...
def get_mqtt_status():
    ...
    try:
        ...
        # is_running é @property — sem parênteses
        running = mqtt_service.is_running
        return jsonify({
            'success': True,
            'running': running,
            'message': 'Servidor MQTT rodando' if running else 'Servidor MQTT parado'
        }), 200
...
def update_mqtt_config():
    ...
    try:
        ...
        plugin = current_app.plugin_manager.get_plugin('plugin_device_manager')
        if not plugin:
            return jsonify({'error': 'Plugin não encontrado'}), 500

        config_path = Path(plugin.plugin_path) / "data" / "mqtt_broker.json"
...
@login_required
def start_mqtt():
    """Inicia o broker MQTT manualmente."""
    try:
        mqtt_service = get_mqtt_service()
        ...
        if not mqtt_service:
            ...
            try:
                plugin = current_app.plugin_manager.get_plugin('plugin_device_manager')
                if plugin:
                    plugin._mqtt_service = mqtt_service
            except Exception:
                pass

            logger.info("Novo MQTTService criado e registrado")
        ...
        if plugin:
            ...
            config_path = (
                ...
                / "plugins" / "plugin_device_manager" / "data" / "mqtt_broker.json"
            )
        ...
        logger.error(f"Erro ao iniciar MQTT: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
...
def mqtt_stop():
    ...
    mqtt_service.stop_broker()
    return jsonify({"success": True, "message": "Broker parado"}), 200
...
def mqtt_subscribe():
    ...
    try:
        ...
        if success:
            return jsonify({'success': True, 'message': f'Inscrito no tópico: {topic}'}), 200
        else:
...
def mqtt_unsubscribe():
    ...
    try:
        ...
        if success:
            return jsonify({'success': True, 'message': f'Desinscrito do tópico: {topic}'}), 200
        else:
...
def mqtt_publish():
    ...
    try:
        ...
        logger.info(f"Publicando MQTT — tópico: {topic}, payload: {payload}")
        success = mqtt_service.publish(topic, payload, qos, retain)

        if success:
            return jsonify({'success': True, 'message': 'Mensagem publicada com sucesso'}), 200
        else:
