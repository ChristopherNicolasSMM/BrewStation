...
async function startMQTT() {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/mqtt/start`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      }
    ...
    if (data.success) {
      ...
      alert("Servidor MQTT iniciado com sucesso");
      loadMQTTStatus();

    } else {

      alert("Erro ao iniciar MQTT: " + (data.error || "Erro desconhecido"));
    ...
    alert("Erro ao iniciar servidor MQTT");
...
async function stopMQTT() {
  ...
  btnStop.innerHTML = '<span class="spinner-border spinner-border-sm"></span> Parando...';

  try {

    const response = await fetch(`${API_BASE}/mqtt/stop`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    if (data.success) {
      ...
      alert('Servidor MQTT parado!');
      loadMQTTStatus();

    } else {

      alert('Erro ao parar MQTT: ' + (data.error || 'Erro desconhecido'));

    }

  } catch (error) {

    console.error('Erro ao parar MQTT:', error);
    alert('Erro ao parar servidor MQTT.');

  } finally {

    btnStop.disabled = false;
    btnStop.innerHTML = '<i class="bi bi-stop-circle"></i> Parar Servidor';
