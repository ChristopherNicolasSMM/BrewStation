...
def notifications():
    if request.method == "POST":
        ...
        notification = create_notification(current_user.id, payload["message"], payload.get("title"))
        return jsonify({"message": "Notificação criada", "notification": notification.to_dict()}), 201
...
def mark_notification_read(notif_id: int):
    try:
        ...
        return jsonify({"message": "Notificação marcada como lida", "notification": notification.to_dict()}), 200
    except ValueError as err:
...
def mark_notification_unread(notif_id: int):
    try:
        ...
        return jsonify({"message": "Notificação marcada como não lida", "notification": notification.to_dict()}), 200
    except ValueError as err:
        return _json_or_error(str(err), 404)
...
def restore_trash_notification(trash_id: int):
    try:
        ...
        return jsonify({"message": "Notificação restaurada", "notification": notification.to_dict()}), 200
    except ValueError as err:
        return _json_or_error(str(err), 404)
