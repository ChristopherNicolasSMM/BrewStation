    ...
    try:
        with zipfile.ZipFile(caminho_backup, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for root, dirs, files in os.walk(pasta_origem):
                ...
                dirs[:] = [d for d in dirs if d not in ['venv', '.venv', 'env', '__pycache__', '.git', '.pytest_cache']]
                
                for file in files:
                    if file.endswith(('.pyc', '.pyo')):
                        continue
                    file_path = os.path.join(root, file)
                    arcname = os.path.relpath(file_path, start=Path(pasta_origem).parent)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print_color("   (ex: C:\\Users\\...\\MeuProjeto ou ./MeuProjeto)", Cores.AMARELO)
        caminho_pasta = input("\n   👉 ").strip()
    ...
    print_color(f"\n   ⚠️  ATENÇÃO: Este script irá MODIFICAR os arquivos da pasta:", Cores.AMARELO)
    ...
    print_color("   - F821 (nome indefinido): adicionar imports faltantes", Cores.AMARELO)
    print_color("   - F841 (variável não usada): verificar lógica (ou remover)", Cores.AMARELO)
    print_color("   - E741 (variável 'l'): renomear para nome descritivo", Cores.AMARELO)
    ...
    print_color(f"   flake8 --exclude=venv,.venv,env,.env,__pycache__,build,dist --max-line-length=88", Cores.VERDE)
